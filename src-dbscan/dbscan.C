...
namespace NWUClustering
{
	...
	int ClusteringAlgo::getClusters_uf( int oclu[] )
	{
         vector <int> clusters;
	...
	{			
		//int tid, i, pid, j, k, npid, root, root1, root2;
                int tid, i, pid, j, npid, root, root1, root2;
				...
				else
				{
					...
					for (j = 0; j < (int) ne.size(); j++)
					{
						...
						if(!dbs.m_visited[npid])
						{
							...
							// enough support
							if ( (int) ne2.size() >= dbs.m_minPts)
							{
								// join
								for(k = 0; k < (int) ne2.size(); k++)
									ne.push_back(ne2[k]);
							}
						}
